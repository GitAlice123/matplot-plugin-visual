...
def export_style(fig: Figure, path: Union[str, Path] = "style_patch.py", source: Optional[str] = None) -> Path:
...
def _marker_from_path_spec(path_spec: Optional[dict[str, Any]]) -> Optional[str]:
    if not path_spec:
        return None
...
def _render_module(patches: list[dict[str, Any]], source: Optional[str] = None) -> str:
